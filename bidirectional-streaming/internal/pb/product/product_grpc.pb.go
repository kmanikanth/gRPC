...
type ProductInfoClient interface {
	...
	BulkAdd(ctx context.Context, opts ...grpc.CallOption) (ProductInfo_BulkAddClient, error)
}
...
func (c *productInfoClient) BulkAdd(ctx context.Context, opts ...grpc.CallOption) (ProductInfo_BulkAddClient, error) {
	stream, err := c.cc.NewStream(ctx, &ProductInfo_ServiceDesc.Streams[2], "/product.ProductInfo/bulkAdd", opts...)
	if err != nil {
		return nil, err
	}
	x := &productInfoBulkAddClient{stream}
	return x, nil
}

type ProductInfo_BulkAddClient interface {
	Send(*Product) error
	Recv() (*BulkAddResponse, error)
	grpc.ClientStream
}

type productInfoBulkAddClient struct {
	grpc.ClientStream
}

func (x *productInfoBulkAddClient) Send(m *Product) error {
	return x.ClientStream.SendMsg(m)
}

func (x *productInfoBulkAddClient) Recv() (*BulkAddResponse, error) {
	m := new(BulkAddResponse)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
	return m, nil
...
type ProductInfoServer interface {
	...
	BulkAdd(ProductInfo_BulkAddServer) error
...
func (UnimplementedProductInfoServer) BulkAdd(ProductInfo_BulkAddServer) error {
...
func _ProductInfo_BulkAdd_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(ProductInfoServer).BulkAdd(&productInfoBulkAddServer{stream})
}

type ProductInfo_BulkAddServer interface {
	Send(*BulkAddResponse) error
	Recv() (*Product, error)
	grpc.ServerStream
}

type productInfoBulkAddServer struct {
	grpc.ServerStream
}

func (x *productInfoBulkAddServer) Send(m *BulkAddResponse) error {
	return x.ServerStream.SendMsg(m)
}

func (x *productInfoBulkAddServer) Recv() (*Product, error) {
	m := new(Product)
	if err := x.ServerStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
var ProductInfo_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "bulkAdd",
			Handler:       _ProductInfo_BulkAdd_Handler,
			ServerStreams: true,
			ClientStreams: true,
		},
	},
